...
async def translate_one(text: str, target: str) -> dict:
    ...
    # 1) Check the cache first. A hit skips the LLM entirely.
    ...
    # 2) Miss: call the LLM (fails loud on error), then store for next time.
...
async def translate(body: TranslateIn, request: Request):
    ...
    log.info(
        "translate",
        extra={"requestId": req_id, "cached": result["cached"], "latencyMs": result["latencyMs"], "chars": len(body.text)},
    )
    return result
